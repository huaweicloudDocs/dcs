...
用户将离线实例数据导入Redis3.x版本的DCS实例。当前版本只有部分Redis3.x版本的DCS实例支持离线导入功能。
...
4.  在需要导入离线实例的DCS缓存实例右侧，单击“操作”栏下的“更多 \> 离线导入”，完成主备切换。
    ...
    页面跳转到DRS的创建离线导入任务页面，具体的迁移操作，请参考DRS服务的[迁移操作](https://support.huaweicloud.com/qs-drs/drs_offline_migration.html)操作。
