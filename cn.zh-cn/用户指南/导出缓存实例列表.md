# 导出缓存实例列表<a name="dcs-zh-ug-181122001"></a>
...
3.  单击左侧菜单栏的“缓存管理”。
4.  设置筛选条件“状态”和“名称”，查询对应的DCS缓存实例。
5.  单击信息栏右上侧的![](figures/icon-export.png)按钮，导出缓存实例列表。

    **图 1**  导出缓存实例列表<a name="fig570113312026"></a>  
    ![](figures/导出缓存实例列表.png "导出缓存实例列表")

    页面左下角显示导出的结果，单击可查看导出的实例信息，导出结果如[图2](#fig9494143610442)所示。

    **图 2**  导出缓存实例列表<a name="fig9494143610442"></a>  
    ![](figures/导出缓存实例列表-4.png "导出缓存实例列表-4")
