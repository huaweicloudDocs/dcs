...
5.  确定选择的区域以及实例是否跨可用区部署。
    ...
    一个区域对应多个可用区（AZ），当前DCS支持将主备实例部署在不同的AZ内，节点间电力与网络均物理隔离。您可以将应用程序也进行跨AZ部署，从而达到数据与应用全部高可用。

    >![](public_sys-resources/icon-note.gif) **说明：**   
    >-   当主备实例进行跨可用区部署时，如果其中一个可用区故障，另一个可用区的节点不受影响。备节点会自动升级为主节点，对外提供服务，从而提供更高的容灾能力。  
    >-   由于实例跨可用区部署时网络访问效率略低于部署在同一可用区内，因此实例跨可用区部署时，主备节点之间同步效率会略有降低。  

6.  确定实例是否配置备份策略。

    当前只有主备实例支持配置备份恢复策略。关于备份恢复，可参考[备份与恢复说明](备份与恢复说明.md)。
...
8.  根据购买前的[准备工作](#section1262819220505)，设置以下基本信息；
    ...
    3.  在“可用区”区域，您可根据实际情况选择。
        ...
        如果“实例类型”选择了主备，页面增加显示“跨可用区部署”参数，您可以选择是否打开跨可用区部署开关。

        如果打开了跨可用区部署开关，页面增加显示“备可用区”参数，您需要在“备可用区”为备实例设置备可用区。
...
9.  设置实例网络环境信息。
    1.  在“虚拟私有云”下拉列表，选择已经创建好的虚拟私有云。
        -   虚拟私有云可以为您的Memcached实例构建隔离的、您可以自主配置和管理的虚拟网络环境。
    ...
    3.  在“IP地址”区域，您可以选择“自动分配”和“手动指定”。
        ...
        -   “手动指定”：选择该项，系统会显示一个输入IP地址输入框，您可以自己输入一个在当前子网下可用的IP，同时，您也可以单击右侧的“查看已使用的IP地址”，系统弹出“查看已使用的IP地址”页面，您可以查看当前子网下的IP使用情况。
...
10. 设置实例密码。
    -   “免密访问”：是一个免密访问的开关。您可以设置访问Memcached实例时是否要进行密码验证。

        >![](public_sys-resources/icon-note.gif) **说明：**   
        >-   打开免密开关，免密模式访问实例，存在安全风险，请谨慎使用。  
        >-   若申请免密模式的Memcached实例，申请成功后，可以通过免密访问进行密码设置，具体可参考[开启Memcached实例的免密访问](开启Memcached实例的免密访问.md)章节。  
...
## 有关购买Memcached实例的常见问题<a name="section6892037104216"></a>

-   [DCS的Memcached与自建Memcached的区别是什么？](https://support.huaweicloud.com/dcs_faq/dcs-zh-ug-180607003.html)
-   [DCS的Memcached兼容的版本号是多少？](https://support.huaweicloud.com/dcs_faq/dcs-zh-ug-180607006.html)
-   [购买DCS的Memcached时如何选择可用区？](https://support.huaweicloud.com/dcs_faq/dcs-zh-ug-180607007.html)
